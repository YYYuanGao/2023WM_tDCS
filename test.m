...
%     angle = rand * pi;
% 
%     sti_final = gray * (1 + Param.Stimuli.GratingContrast * cos(2 * pi * Param.Stimuli.Spatial_freq * (y * sin(angle) + x * cos(angle)) + phase));
%     sti_final(sqrt(x.^2 + y.^2) > Param.Stimuli.OuterSize) = gray; % circle mask
%     mm1 = Screen('MakeTexture', wnd, sti_final);
% 
%     for tempi = 1:36
%         location = Param.Stimuli.Locations(tempi,:);
%         rect = [location(1)-Param.Stimuli.OuterSize, location(2)-Param.Stimuli.OuterSize, location(1)+Param.Stimuli.OuterSize, location(2)+Param.Stimuli.OuterSize];
%         Screen('DrawTexture', wnd, mm1, [], rect);
%     end
% 
