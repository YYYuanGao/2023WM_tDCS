% with feedback
% By @Yuan 20231110 09:44
%% check parameters used
if exist([CurrDir '\Results\training\' SubjID '\' SubjID '_results_sess' num2str(sess_num) '_run' num2str(run_num) '.mat'],'file')
    disp(' ');
...
for trial_i = 1:Param.Trial.TestNum
    ...
    if results.set_size(trial_i) == 1
        ...
        % if results.set_size(trial_i) == 3
        %     b = Theta(randperm(length(Theta), 3));
        %     while min(diff(sort(b))) < 120
        %         b = Theta(randperm(length(Theta), 3));
        %     end
        % elseif results.set_size(trial_i) == 5
        %     b = Theta(randperm(length(Theta), 5));
    ...
    tarx = results.ovalcentre_x1(trial_i)* Param.Settings.PixelPerDegree+Param.Settings.ScrnResolution(3)/2;
    tary = results.ovalcentre_y1(trial_i)* Param.Settings.PixelPerDegree+Param.Settings.ScrnResolution(4)/2;
...
%             results.ori_report(trial_i) =  90 - results.ori_report(trial_i);
        ...
        if ~is_true
            Screen('BlendFunction', wnd, 'GL_SRC_ALPHA', 'GL_ONE_MINUS_SRC_ALPHA');
            Screen('FrameOval',wnd,white,[Param.Settings.ScrnResolution(3)/2-Param.Stimuli.WheelOuter/2,Param.Settings.ScrnResolution(4)/2-Param.Stimuli.WheelOuter/2,Param.Settings.ScrnResolution(3)/2+Param.Stimuli.WheelOuter/2,Param.Settings.ScrnResolution(4)/2+Param.Stimuli.WheelOuter/2],Param.Stimuli.Width,Param.Stimuli.Height);
            Screen('FrameOval', wnd, white, [Param.Settings.ScrnResolution(3)/2-Param.Stimuli.WheelInner/2,Param.Settings.ScrnResolution(4)/2-Param.Stimuli.WheelInner/2,Param.Settings.ScrnResolution(3)/2+Param.Stimuli.WheelInner/2,Param.Settings.ScrnResolution(4)/2+Param.Stimuli.WheelInner/2],Param.Stimuli.Width,Param.Stimuli.Height);
            Screen('FillOval',wnd, Param.Stimuli.Color(results.color_test(trial_i),:), [mousex-Param.Stimuli.OvalSize/2, mousey-Param.Stimuli.OvalSize/2, mousex+Param.Stimuli.OvalSize/2, mousey+Param.Stimuli.OvalSize/2]);
            Screen('Flip',wnd);
        end
    end
    Screen('FillOval',wnd, Param.Stimuli.Color(results.color_test(trial_i),:), [mousex-Param.Stimuli.OvalSize/2, mousey-Param.Stimuli.OvalSize/2, mousex+Param.Stimuli.OvalSize/2, mousey+Param.Stimuli.OvalSize/2]);
    Screen('FrameOval',wnd, Param.Stimuli.Color(results.color_test(trial_i),:), [tarx-Param.Stimuli.OvalSize/2, tary-Param.Stimuli.OvalSize/2, tarx+Param.Stimuli.OvalSize/2, tary+Param.Stimuli.OvalSize/2],Param.Stimuli.Width,Param.Stimuli.Height);
    Screen('Flip',wnd);
    WaitSecs(1);
...
WaitSecs(0.2);
    ...
    %% save the data
    resultsDir = [CurrDir '\Results\training\' SubjID '\'];
    if ~isdir(resultsDir)
        mkdir(resultsDir);
    end
